...
let rangeXStart = moment('2017-06-01 00:00:00', 'YYYY-MM-DD hh:mm:ss');
let rangeXEnd = moment('2017-12-30 23:59:59', 'YYYY-MM-DD hh:mm:ss');

const xScale = d3.scaleTime()
  .domain([rangeXStart, rangeXEnd])
  .range([0, width])
  .clamp(true);
...
function onDrag() {
  ...
  const duration = moment.duration(moment(rangeXStart).diff(moment(rangeXScale.invert(offsetX))));
  xScale.domain([
    moment(rangeXStart).add(duration),
    moment(rangeXEnd).add(duration),
  ]);
  renderAll();
}
function onDragEnd() {
  ...
  const offsetX = d3.event.x - dragStartX;
  const duration = moment.duration(moment(rangeXStart).diff(moment(rangeXScale.invert(offsetX))));
  rangeXStart = moment(rangeXStart).add(duration);
  rangeXEnd = moment(rangeXEnd).add(duration);
  xScale.domain([
    rangeXStart,
    rangeXEnd,
  ]);
